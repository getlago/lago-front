...
async function prepare({ mocks = [rolesListMock] }: { mocks?: TestMocksType } = {}) {
  ...
  await act(() => {
    ref.current?.openDialog({
      invite: { __typename: 'Invite', id: 'invite-123', email: 'test@example.com', roles: ['admin'] },
    })
  })
...
| Test setup | `createRef` + render component | NiceModal.Provider + test wrapper component |
| Test mock | `forwardRef(() => null)` | `{ useMyDialog: () => ({ openMyDialog: jest.fn() }) }` |
