...
**Run every validation command synchronously, in the foreground, and wait for it to finish in this same turn.** `pnpm code:style` and `pnpm test` are slow, but you have exactly one turn: there is no later turn to resume into. Never background a validation command, never "schedule a check-in," never end your turn "waiting for validation to complete." If you do, the run ends with no artifacts and the whole job is discarded as an error. Block on each command, read its result, and only then finish, and your turn must end by writing either the PR artifact set or `impediment.md` (section 6). Emitting neither is a failure, not a hold.
