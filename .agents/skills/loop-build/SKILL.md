...
5. **Implement** per spec.md, inside the `worktree:` path from state.md only (in `in-place` that is the cwd):
   ...
   - **A child that seeds props into `useState` is uncontrolled.** Before wiring one, list every value the parent writes after mount. For each, either remount the child (`key`) or make it read the prop. A parent that resets a value the child also stores is a silent wrong-save: the control shows one branch while the form submits another.
   ...
   - If GraphQL documents change: run `pnpm codegen` and include regenerated files. When the worktree's API runs a FEATURE branch, `git diff src/generated/graphql.tsx` afterwards and keep only the hunks belonging to your own operations: CI regenerates from lago-api `main`, so anything the branch adds or rolls back is drift that will not reproduce.
