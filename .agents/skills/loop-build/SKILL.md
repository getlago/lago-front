...
5. **Implement** per spec.md, inside the `worktree:` path from state.md only (in `in-place` that is the cwd):
   ...
   - **Comments**: `.agents/docs/typescript-conventions.md` → "Comments: Default to None" is binding. Before handing off, reread every comment the diff adds and delete it unless it names an external constraint the reader cannot see, explains why NOT the obvious alternative, or flags a trap that bites on edit. Cap what survives at 1-2 lines. Never copy the same comment into a second file: a rule that needs restating in N places belongs in CLAUDE.md or `.agents/docs/`, and the code just follows it.
