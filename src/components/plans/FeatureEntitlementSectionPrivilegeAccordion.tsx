...
import { ComboBox, ComboboxItem, MultipleComboBox, TextInput } from '~/components/form'
import {
...
gql`
  ...
  query getFeatureDetailsForFeatureEntitlementPrivilegeSection($id: ID!) {
    feature(id: $id) {
...
> = ({ formikProps, entitlement, isInitiallyOpen }) => {
  ...
  if (valueType === PrivilegeValueTypeEnum.Select) {
    return (
      <MultipleComboBox
        ...
        onChange={(newValue) => {
          onChange(newValue.length ? newValue.map((v) => v.value).join(',') : undefined)
        }}
      />
    )
  }
