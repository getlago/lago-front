...
const usageChargeDrawerSchema = z
  .object({
    billableMetricId: z.string().min(1, { message: 'text_1771342994699klxu2paz7g8' }),
            ...
            {isCreatePickerScreen ? (
              ...
              <CenteredPage.SubsectionWrapper>
                ...
                <CenteredPage.PageSection>
                  ...
                  {(!!formValues.properties || !!formValues.filters?.length) && (
                    <div
                      className={tw(
                        'flex flex-col gap-4',
                        !!formValues.billableMetric?.filters?.length && 'mt-6 px-4',
                      )}
                    >
                      {/* Simple charge or default property for groups */}
                      {!!formValues.properties && (
                        <ConditionalWrapper
                          condition={!!formValues.billableMetric?.filters?.length}
                          invalidWrapper={(children) => (
                            <div data-test="default-usage-charge-without-filters">{children}</div>
                          )}
                          validWrapper={(children) => {
                            ...
                            return (
                              <Accordion
                                noContentMargin
                                className={CHARGE_DEFAULT_PROPERTY_ID}
                                summary={
                                ...
                                data-test={CHARGE_DEFAULT_PROPERTY_ID}
                              >
                                {children}
                              </Accordion>
                            )
                  ...
                  {/* Add filter / Add default property buttons */}
                        ...
                        <Button
                          ...
                          onClick={() => {
                            ...
                            scrollToAndClickElement({
                              selector: `.${CHARGE_DEFAULT_PROPERTY_ID} .${MUI_BUTTON_BASE_ROOT_CLASSNAME}`,
                            })
                          }}
                        >
