...
import { FlutterwaveIntegrationDetailsFragment } from '~/generated/graphql'
import { AllTheProviders, render } from '~/test-utils'

import { useAddFlutterwaveDialog } from '../AddFlutterwaveDialog'
...
jest.mock('~/generated/graphql', () => ({
  ...
  useUpdateFlutterwavePaymentProviderMutation: () => [mockUpdate],
  useGetProviderByCodeForFlutterwaveLazyQuery: () => [mockGetProviderByCode],
  useAddFlutterwavePaymentProviderMutation: () => [mockAdd],
  useDeleteFlutterwaveIntegrationMutation: () => [mockDelete],
}))
...
const provider: FlutterwaveIntegrationDetailsFragment = {
  __typename: 'FlutterwaveProvider',
  id: 'flutterwave-provider-id',
...
describe('useAddFlutterwaveDialog', () => {
  afterEach(cleanup)
  ...
  describe('GIVEN the code is already taken by another provider', () => {
    const submitEditionWithTakenCode = async (): Promise<() => Promise<unknown>> => {
      ...
      const { result } = renderHook(() => useAddFlutterwaveDialog(), { wrapper })

      act(() => {
        result.current.openAddFlutterwaveDialog({ provider })
      })
