...
import { AddCashfreeProviderDialogFragment } from '~/generated/graphql'
import { AllTheProviders, render } from '~/test-utils'

import { useAddCashfreeDialog } from '../AddCashfreeDialog'
...
jest.mock('~/generated/graphql', () => ({
  ...
  useUpdateCashfreeApiKeyMutation: () => [mockUpdate],
  useGetProviderByCodeForCashfreeLazyQuery: () => [mockGetProviderByCode],
  useAddCashfreeApiKeyMutation: () => [mockAdd],
  useDeleteCashfreeMutation: () => [mockDelete],
}))
...
const provider: AddCashfreeProviderDialogFragment = {
  __typename: 'CashfreeProvider',
  id: 'cashfree-provider-id',
...
describe('useAddCashfreeDialog', () => {
  ...
  describe('GIVEN the code is already taken by another provider', () => {
    const submitEditionWithTakenCode = async (): Promise<() => Promise<unknown>> => {
      ...
      const { result } = renderHook(() => useAddCashfreeDialog(), { wrapper })

      act(() => {
        result.current.openAddCashfreeDialog({ provider })
      })
