...
import { AddMoneyhashProviderDialogFragment } from '~/generated/graphql'
import { AllTheProviders, render } from '~/test-utils'

import { useAddMoneyhashDialog } from '../AddMoneyhashDialog'
...
jest.mock('~/generated/graphql', () => ({
  ...
  useUpdateMoneyhashApiKeyMutation: () => [mockUpdate],
  useGetProviderByCodeForMoneyhashLazyQuery: () => [mockGetProviderByCode],
  useAddMoneyhashApiKeyMutation: () => [mockAdd],
  useDeleteMoneyhashIntegrationMutation: () => [mockDelete],
}))
...
const provider: AddMoneyhashProviderDialogFragment = {
  __typename: 'MoneyhashProvider',
  id: 'moneyhash-provider-id',
...
describe('useAddMoneyhashDialog', () => {
  afterEach(cleanup)
  ...
  describe('GIVEN the code is already taken by another provider', () => {
    const submitEditionWithTakenCode = async (): Promise<() => Promise<unknown>> => {
      ...
      const { result } = renderHook(() => useAddMoneyhashDialog(), { wrapper })

      act(() => {
        result.current.openAddMoneyhashDialog({ provider })
      })
