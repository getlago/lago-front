...
export const SubscriptionActivationRuleSection = withForm({
  ...
  props: {
    customerExternalId: '',
    formType: FORM_TYPE_ENUM.creation as keyof typeof FORM_TYPE_ENUM,
    subscriptionStatus: undefined as StatusTypeEnum | null | undefined,
  },
  render: function Render({ form, customerExternalId, formType, subscriptionStatus }) {
    ...
    const paymentMethodValue = useStore(form.store, (s) => s.values.paymentMethod)
    const activationRuleType = useStore(form.store, (s) => s.values.activationRuleType)
    ...
    useEffect(() => {
      if (isActivationRuleDisabled) {
        form.setFieldValue('activationRuleType', ActivationRuleFormEnum.Immediately)
      }
          ...
          <form.AppField name="activationRuleTimeoutHours">
            {(field) => (
              <field.TextInputField
                ...
                data-test={ACTIVATION_RULE_TIMEOUT_INPUT_TEST_ID}
              />
            )}
          </form.AppField>
        )}
