...
// `terminateContract` is also the cancel mutation: it cancels a pending contract and terminates
// an active one. Anything already terminated or canceled is rejected by the API.
...
type UseTerminateContractDialogReturn = {
  openTerminateContractDialog: (contract?: TerminateContractDialogProps) => void
}

export const useTerminateContractDialog = (): UseTerminateContractDialogReturn => {
  const centralizedDialog = useCentralizedDialog()
  const { translate } = useInternationalization()

  const [terminateContract] = useTerminateContractMutation()

  const openTerminateContractDialog = (contract?: TerminateContractDialogProps): void => {
    if (!contract) return
