                  ...
                  <div className="flex flex-col gap-6">
                    {displayChargeIdField && (
                      <ComboBox
                        ...
                        onChange={(selectedChargeId) => {
                          const isUsageCharge =
                            currentInvoiceSubscription?.subscription.plan.charges?.find(
                              (charge) => charge.id === selectedChargeId,
                            )

                          const isFixedCharge =
                            currentInvoiceSubscription?.subscription.plan.fixedCharges?.find(
                              (fixedCharge) => fixedCharge.id === selectedChargeId,
                            )

                          if (isUsageCharge) {
                            formikProps.setFieldValue('chargeId', selectedChargeId || '')
                          } else if (isFixedCharge) {
                            formikProps.setFieldValue('fixedChargeId', selectedChargeId || '')
                          }
                        }}
                        value={
                          formikProps.values.chargeId || formikProps.values.fixedChargeId || ''
