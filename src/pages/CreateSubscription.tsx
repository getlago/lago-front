...
import {
  ...
  FeatureEntitlementForSubscriptionFragmentDoc,
  PlanInterval,
  StatusTypeEnum,
  TimezoneEnum,
...
? Ecraser le entitlements du plan quand on edit une sub?
...
gql`
  ...
  query getSubscriptionForCreateSubscription($id: ID!) {
    subscription(id: $id) {
      ...
      ...FeatureEntitlementForSubscription
    }
  }
...
const CreateSubscription = () => {
    ...
    initialValues: {
      ...
      entitlements:
        subscription?.entitlements?.map(
          ({ name, code, feature, privileges, ...restEntitlement }) => ({
  ...
  const selectedPlan = useMemo(() => {
    ...
    const foundPlan = (planData?.plans?.collection || []).find((p) => p.id === subscriptionPlanId)

    // We erase the plan entitlement with the subscription entitlement if they exist
    // The form modify the plan entitlement, not the subscription entitlement
    // But on same we'll send the entitlement within the subscription object, not the plan one
    if (foundPlan?.entitlements && !!subscription?.entitlements?.length) {
      foundPlan.entitlements = subscription.entitlements?.map(
        ({ privileges: entitlementPrivileges, ...restEntitlement }) => ({
          ...restEntitlement,
          __typename: 'PlanEntitlement',
          privileges: entitlementPrivileges.map((restPrivilege) => ({
            ...restPrivilege,
    ...
    return foundPlan
  }, [planData?.plans?.collection, subscription?.entitlements, subscriptionPlanId])
