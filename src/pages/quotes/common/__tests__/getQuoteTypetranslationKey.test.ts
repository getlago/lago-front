...
import { getQuoteTypeTranslationKey } from '../getQuoteTypetranslationKey'

describe('getQuoteTypeTranslationKey', () => {
  it.each([
    ...
    const result = getQuoteTypeTranslationKey(orderType)
  ...
  it('GIVEN different order types, THEN should return different keys for one-off and amendment', () => {
    const oneOffKey = getQuoteTypeTranslationKey(OrderTypeEnum.OneOff)
    const amendmentKey = getQuoteTypeTranslationKey(OrderTypeEnum.SubscriptionAmendment)

    expect(oneOffKey).not.toBe(amendmentKey)
  })

  it('GIVEN subscription creation, THEN should return the default key', () => {
    const creationKey = getQuoteTypeTranslationKey(OrderTypeEnum.SubscriptionCreation)
    const oneOffKey = getQuoteTypeTranslationKey(OrderTypeEnum.OneOff)
    const amendmentKey = getQuoteTypeTranslationKey(OrderTypeEnum.SubscriptionAmendment)
