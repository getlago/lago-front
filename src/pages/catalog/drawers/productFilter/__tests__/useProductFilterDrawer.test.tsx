...
import {
  CreateProductFilterDocument,
  ProductFilterForDrawerFragment,
  UpdateProductFilterDocument,
...
jest.mock('../ProductFilterDrawerContent', () => ({
  ProductFilterDrawerContent: ({
    ...
    <>
      ...
      <span data-test="product-item-id-value">{form.state.values.productId ?? ''}</span>
      <span data-test="values-count">{form.state.values.values?.length ?? 0}</span>
    </>
  ),
}))

const productFilterFixture: ProductFilterForDrawerFragment = {
  id: 'pif-1',
  name: 'Storage EU',
  code: 'storage_eu',
  ...
  product: { id: 'pi-1', name: 'Storage', code: 'storage' },
  values: [
    {
      id: 'v-1',
      value: 'card',
...
describe('useProductFilterDrawer', () => {
  beforeEach(() => {
    jest.clearAllMocks()
    lastDrawerArgs = null
  })
  ...
  describe('GIVEN create mode (no product filter argument)', () => {
    ...
    it('creates the filter, closes, navigates to details and toasts', async () => {
      const { result } = renderDrawerHook([createProductFilterMock()])

      act(() => result.current.openDrawer())
      await seedAndSubmit()

      await waitFor(() => expect(mockClose).toHaveBeenCalledTimes(1))
      expect(mockNavigate).toHaveBeenCalledWith('/product-catalog/product-filters/pif-1/overview')
    ...
    it('keeps the drawer open on a duplicate code without toasting', async () => {
      const { result } = renderDrawerHook([
        createProductFilterMock({ data: null, errors: [duplicateCodeError] }),
      ])
    ...
    it('seeds the values editor with the prefilled product filters (attachToProduct)', () => {
      ...
      expect(contentProps?.productSeed).toEqual({ value: 'pi-9', label: 'Storage' })
  ...
  describe('GIVEN edit mode (a product filter argument)', () => {
    ...
    it('updates the filter without productId, closes and toasts without navigating', async () => {
      const updateVars = jest.fn()
      const { result } = renderDrawerHook([
        {
          request: { query: UpdateProductFilterDocument },
          variableMatcher: (vars) => {
            updateVars(vars)
          ...
          result: {
            data: {
              updateProductFilter: { ...productFilterFixture, name: 'Storage EU 2' },
            },
          },
        },
      ])
      ...
      // productId is create-only and must not be sent on update.
