...
import { useProductCategoryDrawer } from '../useProductCategoryDrawer'
...
const productCategoryFixture: ProductCategoryForProductCategoryDrawerFragment = {
  id: 'prod-1',
  name: 'Object storage',
  code: 'object_storage',
...
const createProductCategoryMockFactory = (
  overrides: Partial<MockedResponse['result']> = {},
): MockedResponse => ({
  request: { query: CreateProductCategoryDocument },
...
describe('useProductCategoryDrawer', () => {
  ...
  describe('GIVEN create mode (no productCategory argument)', () => {
    ...
    it('creates the productCategory then closes, navigates to its details and toasts', async () => {
      const { result } = renderDrawerHook([createProductCategoryMockFactory()])
    ...
    it('keeps the drawer open, resets the form and links the productCategory when create more is on', async () => {
      const { result } = renderDrawerHook([createProductCategoryMockFactory()])
      ...
      await waitFor(() =>
        expect(addToast).toHaveBeenCalledWith({
          severity: 'success',
          message:
            'text_17836270312838hlfh44gw4i|Storage|/acme/product-catalog/product-categories/prod-1/overview',
        }),
      )
    ...
    it('escapes double quotes in the productCategory name interpolated into the linked toast', async () => {
      const { result } = renderDrawerHook([
        {
          request: { query: CreateProductCategoryDocument },
    ...
    it('surfaces a duplicate code under the code input and keeps the drawer open', async () => {
      const { result } = renderDrawerHook([
        createProductCategoryMockFactory({ data: null, errors: [duplicateCodeError] }),
      ])
  ...
  describe('GIVEN edit mode (a productCategory argument)', () => {
    ...
    it('prefills the form with the productCategory values', async () => {
      const { result } = renderDrawerHook()
      ...
      expect(screen.getByDisplayValue('Base storage productCategory')).toBeInTheDocument()
      expect(screen.getByDisplayValue('Storage')).toBeInTheDocument()
    })

    it('locks the code input when the productCategory is attached to a plan or subscription', async () => {
      const { result } = renderDrawerHook()
    ...
    it('updates the productCategory, closes the drawer and toasts without navigating', async () => {
      const { result } = renderDrawerHook([
        {
          request: { query: UpdateProductCategoryDocument },
          variableMatcher: (vars) =>
            vars?.input?.id === 'prod-1' &&
            vars?.input?.name === 'Object storage EU' &&
            vars?.input?.code === 'object_storage' &&
            vars?.input?.description === 'Base storage productCategory' &&
            vars?.input?.invoiceDisplayName === 'Storage',
          result: {
            data: {
              updateProductCategory: { ...productCategoryFixture, name: 'Object storage EU' },
            },
          },
        },
      ])
    ...
    it('clears an emptied description by sending null instead of undefined', async () => {
      ...
      const { result } = renderDrawerHook([
        {
          ...
          result: {
            data: {
              updateProductCategory: { ...productCategoryFixture, description: null },
            },
          },
        },
      ])
      ...
      const descriptionInput = await screen.findByDisplayValue('Base storage productCategory')
  ...
  describe('GIVEN the optional description field', () => {
    it('reveals and removes a prefilled description in edit mode', async () => {
      ...
      expect(screen.queryByDisplayValue('Base storage productCategory')).not.toBeInTheDocument()
