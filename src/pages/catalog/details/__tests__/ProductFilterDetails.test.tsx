...
import { GetProductFilterForDetailsDocument } from '~/generated/graphql'
import { AllTheProviders, testMockNavigateFn } from '~/test-utils'

import ProductFilterDetails, {
...
jest.mock('~/pages/catalog/dialogs/useDeleteProductFilterDialog', () => ({
  useDeleteProductFilterDialog: () => ({
    openDeleteProductFilterDialog: mockOpenDeleteProductFilterDialog,
  }),
}))

jest.mock('../ProductFilterDetailsOverview', () => ({
  __esModule: true,
  default: () => null,
}))

jest.mock('../ProductFilterActivityLogs', () => ({
  __esModule: true,
  default: () => null,
}))
...
const detailsQueryMock = {
  ...
  result: { data: { productFilter: productFilterFixture } },
}

const ProductFilterDetailsWithHeader = () => (
  <>
    <MainHeader />
...
const renderPage = () =>
  rtlRender(<ProductFilterDetailsWithHeader />, {
    wrapper: ({ children }) => (
      <AllTheProviders
        forceTypenames
        mocks={[detailsQueryMock]}
        useParams={{ productFilterId: 'pif-1', tab: 'overview' }}
      >
        {children}
      </AllTheProviders>
    ),
...
describe('ProductFilterDetails', () => {
  ...
  it('renders the catalog breadcrumb link and the static grey product filter crumb', async () => {
    ...
    expect(catalogCrumb).toHaveAttribute('href', '/product-catalog/product-filters')
  })
  ...
  it('renders the RateCardPreview scoped to this product filter when that tab is active', async () => {
    ...
    await waitFor(() => {
      expect(mockRateCardPreviewProps).toHaveBeenCalledWith({
        scope: {
          productFilter: expect.objectContaining({
            id: 'pif-1',
            name: 'Region filter',
            product: expect.objectContaining({ id: 'pitem-1', name: 'Seats' }),
          }),
        },
      })
    })
  ...
  it('opens the edit drawer with the loaded item filter from the actions dropdown', async () => {
    ...
    expect(mockOpenEditProductFilterDrawer).toHaveBeenCalledWith(
      expect.objectContaining({
        productFilter: expect.objectContaining({ id: 'pif-1', code: 'region_filter' }),
      }),
    )
  })
  ...
  it('opens the delete dialog whose callback navigates back to the product filters list', async () => {
    ...
    expect(mockOpenDeleteProductFilterDialog).toHaveBeenCalledWith(
      expect.objectContaining({
        productFilter: expect.objectContaining({ id: 'pif-1' }),
      }),
    )

    const { callback } = mockOpenDeleteProductFilterDialog.mock.calls[0][0]

    callback()

    expect(testMockNavigateFn).toHaveBeenCalledWith('/product-catalog/product-filters')
  })
