...
jest.mock('~/pages/catalog/drawers/productFilter/useProductFilterDrawer', () => ({
  useProductFilterDrawer: () => ({ openDrawer: mockOpenDrawer }),
}))

jest.mock('~/pages/catalog/dialogs/useDeleteProductFilterDialog', () => ({
  useDeleteProductFilterDialog: () => ({
    openDeleteProductFilterDialog: mockOpenDeleteDialog,
  }),
}))
...
const productWithFilters: ProductForFilterPreviewFragment = {
  __typename: 'Product',
  id: PRODUCT_ITEM_ID,
  name: 'Seats',
  code: 'seats',
...
const productWithoutFilters: ProductForFilterPreviewFragment = {
  __typename: 'Product',
  id: PRODUCT_ITEM_ID,
  name: 'Seats',
  code: 'seats',
...
const buildRow = (index: number): ProductFilterForListFragment => ({
...
const filtersQueryMock = (
  ...
  collection: ProductFilterForListFragment[],
  totalCount: number,
): MockedResponse => ({
  request: { query: GetProductFiltersForProductDetailsDocument, variables },
  result: {
    data: {
      productFilters: {
        __typename: 'ProductFilterCollection',
...
const renderPreview = (
  ...
  product: ProductForFilterPreviewFragment = productWithFilters,
) =>
  rtlRender(<ProductFilterPreview product={product} />, {
    wrapper: ({ children }) => (
...
describe('ProductFilterPreview', () => {
  ...
  it('renders up to the preview limit of rows returned by the scoped query', async () => {
    ...
    await act(() =>
      renderPreview([filtersQueryMock({ productId: PRODUCT_ITEM_ID, limit: 7 }, collection, 7)]),
    )
  ...
  it('shows the view-all link deep-linked to this productCategory item when the total exceeds the preview limit', async () => {
    ...
    await act(() =>
      renderPreview([filtersQueryMock({ productId: PRODUCT_ITEM_ID, limit: 7 }, collection, 12)]),
    )
    ...
    expect(decodeURIComponent(anchor?.getAttribute('href') ?? '')).toContain(
      'pif_productFilterProduct=pi-1|-_-|Seats',
    )
    expect(anchor?.getAttribute('href')).toContain('/product-catalog/product-filters')
  })
  ...
  it('hides the view-all link when the total fits within the preview limit', async () => {
    ...
    await act(() =>
      renderPreview([filtersQueryMock({ productId: PRODUCT_ITEM_ID, limit: 7 }, collection, 5)]),
    )
