...
const sortThresholds = (thresholds: AlertThreshold[]): AlertThreshold[] => {
  const recurringThreshold = thresholds.find((threshold) => threshold.recurring)
  const nonRecurringThresholds = thresholds.filter((threshold) => !threshold.recurring)
  ...
  const formikProps = useFormik<CreateSubscriptionAlertInput>({
    initialValues: {
      ...
      thresholds: !!existingAlert?.thresholds?.length
        ? sortThresholds(existingAlert?.thresholds)
        : [
            {
              code: '',
              recurring: false,
    ...
    onSubmit: async ({ billableMetricId, alertType, ...values }) => {
      // Edition
      if (!!existingAlert?.id) {
        await updateAlert({
          variables: {
